...
operator fun <T> Spek<T, Number>.plus(other: Spek<T, out Number>)
        = BinaryOperationSpek<T, Number, Number>(this, other as Spek<T, Number>) { a, b -> a().toDouble() + b().toDouble() }
operator fun <T> Spek<T, Number>.plus(other: Number)
        = BinaryOperationSpek<T, Number, Number>(this, ValueSpek(other)) { a, b -> a().toDouble() + b().toDouble() }

operator fun <T> Spek<T, Number>.minus(other: Spek<T, out Number>)
        = BinaryOperationSpek<T, Number, Number>(this, other as Spek<T, Number>) { a, b -> a().toDouble() - b().toDouble() }
operator fun <T> Spek<T, Number>.minus(other: Number)
        = BinaryOperationSpek<T, Number, Number>(this, ValueSpek(other)) { a, b -> a().toDouble() - b().toDouble() }
...
class LargestSpek<T>(private vararg val bases: Spek<T, out Number>) :
    Spek<T, Number> {
...
class SmallestSpek<T>(private vararg val bases: Spek<T, Number>) :
    Spek<T, Number> {
